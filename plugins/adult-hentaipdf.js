...
const handler = async (m, {conn, text, usedPrefix, command, args}) => {
  ...
  if (!text) throw `✧ Ingresa el nombre de alguna categoría de hentai, Ejemplo: ${usedPrefix + command} Yumeko`;
  try {
    m.reply(global.wait);
    ...
    throw `*✧ Error, vuelva a intentarlo y pruebe con otra categoría*`;
  }
};
...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[✧ INFO ✧] 𝙻𝙾𝚂 𝙲𝙾𝙼𝙰𝙽𝙳𝙾𝚂 +𝟷𝟾 𝙴𝚂𝚃𝙰𝙽 𝙳𝙴𝚂𝙰𝙲𝚃𝙸𝚅𝙰𝙳𝙾𝚂 𝙴𝙽 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾, 𝚂𝙸 𝙴𝚂 𝙰𝙳𝙼𝙸𝙽 𝚈 𝙳𝙴𝚂𝙴𝙰 𝙰𝙲𝚃𝙸𝚅𝙰𝚁𝙻𝙾𝚂 𝚄𝚂𝙴 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 .enable modohorny*'
if (!text) throw `*✧ Ingresa el nombre de alguna categoría de hentai, Ejemplo: ${usedPrefix + command} Yumeko*`
try {
m.reply(global.wait)
